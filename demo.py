...
def main_frame(root):
    ...
    choices = {'Tfc_i': tk.DoubleVar(frame), 'Pa_des_i': tk.DoubleVar(frame), 'Pc_des_i': tk.DoubleVar(frame),
               ...
               't0_step_i': tk.DoubleVar(frame, 0), 'tf_step_i': tk.DoubleVar(frame, 500),
               'delta_t_load_i': tk.DoubleVar(frame, 20), 'i_ini_i': tk.DoubleVar(frame, 0.4),
               'i_final_i': tk.DoubleVar(frame, 0.8), 'i_pola_i': tk.DoubleVar(frame, 0.5),
               'i_EIS_i': tk.DoubleVar(frame, 0.5), 't_purge_i': tk.DoubleVar(frame, 0.6),
    ...
    tk.Label(frame, text='Fuel cell:', fg='black', font=('Times New Roman', 12, 'bold')).grid(row=0, column=1)
    ...
    tk.Button(frame, text='Step curve',
        ...
        .grid(row=26, column=1, padx=10, pady=20)
    ...
    tk.Button(frame, text='Pola curve', bg='white',
        ...
        .grid(row=26, column=2, padx=10, pady=20)
    ...
    tk.Button(frame, text='EIS curve', bg='brown',
        ...
        .grid(row=26, column=3, padx=10, pady=20)

    # About button
    tk.Button(frame, text='About', bg='black', command=about, fg='white') \
        .grid(row=26, column=4, ipadx=12)
...
def show(choices, button_type):
    ...
    Tfc, Pa_des, Pc_des, Sa, Sc, Phi_a_des, Phi_c_des, Aact, Hgdl, Hcl, Hmem, Hgc, Wgc, Lgc, epsilon_gdl, epsilon_mc, \
        tau, epsilon_c, e, Re, i0_c_ref, kappa_co, kappa_c, a_slim, b_slim, a_switch, C_dl, t_step, i_step, i_pola, \
        i_EIS, t_purge, delta_t_purge, type_fuel_cell, type_auxiliary, type_purge, max_step, type_display, type_plot \
    ...
    if button_type == 0:
        ...
        AlphaPEM(current_density, Tfc, Pa_des, Pc_des, Sa, Sc, Phi_a_des, Phi_c_des, t_step, i_step, i_pola, delta_pola,
                 i_EIS, ratio_EIS, t_EIS, f_EIS, Aact, Hgdl, Hmem, Hcl, Hgc, Wgc, Lgc, epsilon_gdl, tau, epsilon_mc,
                 epsilon_c, e, Re, i0_c_ref, kappa_co, kappa_c, a_slim, b_slim, a_switch, C_dl, max_step, n_gdl,
                 t_purge, type_fuel_cell, type_current, type_auxiliary, type_control, type_purge, type_display,
                 type_plot)
...
def about():
    ...
    """
    msg = "AlphaPEM is an open-source PEM fuel cell simulator for control system applications. It is a physical, dynamic, two-phase, isothermal, 1D model." \
