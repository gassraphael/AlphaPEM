...
function sigma_e_eff(element::Symbol,
    ...
    if element == :gdl  # The effective electrical conductivity at the GDL.
        ...
        sigma_e_gdl = pp.sigma_e_gdl  # Electrical conductivity of the GDL.
...
function calculate_rho_Cp0(element::Symbol,
    ...
    if element == :agdl || element == :cgdl || element == :ampl || element == :cmpl
        ...
        if element == :agdl || element == :cgdl  # In the GDLs.
            rho_gdl, Cp_gdl = pp.rho_gdl, pp.Cp_gdl
            return average((rho_gdl * Cp_gdl, rho_H2O_l(T) * Cp0(:H2O_l, T), rho_Cp0_gaz),
                           _safe_porous_phase_weights(epsilon, s))
        else  # In the MPLs.
            rho_mpl, Cp_mpl = pp.rho_mpl, pp.Cp_mpl
    ...
    elseif element == :mem  # The volumetric heat capacity at the membrane.
        ...
        rho_mem, Cp_mem = pp.rho_mem, pp.Cp_mem
        fv_val = fv(lambdaa, T, pp)
