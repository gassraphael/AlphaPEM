...
function calculate_heat_int_values!(heat_int_work::MEAHeatIntWorkspace,
    ...
    k_th_eff_agc_agdl = k_th_eff(:agdl, T_agdl[1], C_v_agdl[1], s_agdl[1], nothing,
                                 C_H2_agdl[1], nothing, C_N2_agdl[1], epsilon_gdl, nothing, epsilon_c)
    ...
    @inbounds for i in 1:(nb_gdl - 1)
        k_th_eff_agdl_agdl[i] = hmean(
            k_th_eff(:agdl, T_agdl[i], C_v_agdl[i], s_agdl[i], nothing,
                     C_H2_agdl[i], nothing, C_N2_agdl[i], epsilon_gdl, nothing, epsilon_c),
            k_th_eff(:agdl, T_agdl[i+1], C_v_agdl[i+1], s_agdl[i+1], nothing,
                     C_H2_agdl[i+1], nothing, C_N2_agdl[i+1], epsilon_gdl, nothing, epsilon_c)
        )
    end

    k_th_eff_agdl_ampl = hmean(
        k_th_eff(:agdl, T_agdl[nb_gdl], C_v_agdl[nb_gdl], s_agdl[nb_gdl], nothing,
             C_H2_agdl[nb_gdl], nothing, C_N2_agdl[nb_gdl], epsilon_gdl, nothing, epsilon_c),
        k_th_eff(:ampl, T_ampl[1], C_v_ampl[1], s_ampl[1], nothing, C_H2_ampl[1],
             nothing, C_N2_ampl[1], epsilon_mpl),
    ...
    @inbounds for i in 1:(nb_mpl - 1)
        k_th_eff_ampl_ampl[i] = hmean(
            k_th_eff(:ampl, T_ampl[i], C_v_ampl[i], s_ampl[i], nothing,
                     C_H2_ampl[i], nothing, C_N2_ampl[i], epsilon_mpl),
            k_th_eff(:ampl, T_ampl[i+1], C_v_ampl[i+1], s_ampl[i+1], nothing,
                     C_H2_ampl[i+1], nothing, C_N2_ampl[i+1], epsilon_mpl)
        )
    end

    k_th_eff_ampl_acl = hmean(
        k_th_eff(:ampl, T_ampl[nb_mpl], C_v_ampl[nb_mpl], s_ampl[nb_mpl], nothing,
             C_H2_ampl[nb_mpl], nothing, C_N2_ampl[nb_mpl], epsilon_mpl),
        k_th_eff(:acl, T_acl, C_v_acl, s_acl, lambda_acl, C_H2_acl, nothing, C_N2_acl, nothing, Hacl),
        Hmpl_node / 2, Hacl / 2
    )
    ...
    k_th_eff_mem_ccl = hmean(
        k_th_eff(:mem, T_mem, nothing, nothing, lambda_mem),
        k_th_eff(:ccl, T_ccl, C_v_ccl, s_ccl, lambda_ccl, nothing, C_O2_ccl, C_N2_ccl, nothing, Hccl),
        Hmem / 2, Hccl / 2
    )
    ...
    @inbounds for i in 1:(nb_gdl - 1)
        k_th_eff_cgdl_cgdl[i] = hmean(
            k_th_eff(:cgdl, T_cgdl[i], C_v_cgdl[i], s_cgdl[i], nothing,
                     nothing, C_O2_cgdl[i], C_N2_cgdl[i], epsilon_gdl, nothing, epsilon_c),
            k_th_eff(:cgdl, T_cgdl[i+1], C_v_cgdl[i+1], s_cgdl[i+1], nothing,
                     nothing, C_O2_cgdl[i+1], C_N2_cgdl[i+1], epsilon_gdl, nothing, epsilon_c)
        )
    end
...
"""
function sigma_p_eff(element::Symbol,
                     lambdaa,
                     T,
...
"""
function sigma_e_eff(element::Symbol,
...
"""
function k_th_gaz_mixture(k_th_g, mu_g, x, M)
...
"""
function k_th_eff(element::Symbol,
                  T,
                  C_v=nothing,
                  s=nothing,
                  lambdaa=nothing,
...
"""
function calculate_rho_Cp0(element::Symbol,
                           T,
                           C_v=nothing,
                           s=nothing,
                           lambdaa=nothing,
