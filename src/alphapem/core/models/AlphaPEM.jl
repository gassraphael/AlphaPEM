...
function _check_simulation_preconditions!(simu::AlphaPEM)
    ...
    if is_first_run
        if startswith(string(simu.cfg.type_fuel_cell), "EH")
            println("Warning: EH-Group fuel cell examples may be outdated. Using ZSW is recommended.\n")
        end
        if startswith(string(simu.cfg.type_fuel_cell), "ZSW") && simu.cfg.type_flow == :co_flow
