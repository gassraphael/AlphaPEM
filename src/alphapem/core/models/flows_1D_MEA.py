...
def calculate_flows_1D_MEA(sv_1D, i_fc, v_a, v_c, operating_inputs, parameters):
    ...
    (H_gdl_node, H_mpl_node, Pagc, Pcgc, Pcap_agdl, Pcap_cgdl, rho_agc, rho_cgc, D_eff_EOD_acl_mem, D_eff_EOD_mem_ccl,
     ...
     T_acl_mem_ccl) = flows_1D_MEA_int_values(sv_1D, i_fc, parameters)
    ...
    # Anode side
    Jl_agc_agdl = - theta_l_rem * epsilon_gdl * sv_1D['s_agdl_1'] * max((Pcap_agdl + rho_agc * v_a ** 2 / 2), 0)
    Jl_agdl_agdl = [None] + [- D_cap_agdl_agdl[i] * d_dx(y_minus = sv_1D[f's_agdl_{i}'], y_plus = sv_1D[f's_agdl_{i + 1}'],
                                                         dx = H_gdl_node / 2)
                             for i in range(1, nb_gdl)]
    Jl_agdl_ampl = - D_cap_agdl_ampl * d_dx(y_minus = sv_1D[f's_agdl_{nb_gdl}'], y_plus = sv_1D['s_ampl_1'],
                                            dx_minus = H_gdl_node / 2, dx_plus = H_mpl_node / 2)
    Jl_ampl_ampl = [None] + [- D_cap_ampl_ampl[i] * d_dx(y_minus = sv_1D[f's_ampl_{i}'], y_plus = sv_1D[f's_ampl_{i + 1}'],
                                                         dx = H_mpl_node / 2)
                             for i in range(1, nb_mpl)]
    Jl_ampl_acl = - D_cap_ampl_acl * d_dx(y_minus = sv_1D[f's_ampl_{nb_mpl}'], y_plus = s_acl,
                                          dx_minus = H_mpl_node / 2, dx_plus = Hacl / 2)

    # Cathode side
    Jl_ccl_cmpl = - D_cap_ccl_cmpl * d_dx(y_minus = s_ccl, y_plus = sv_1D['s_cmpl_1'],
                                          dx_minus = Hccl / 2, dx_plus = H_mpl_node / 2)
    Jl_cmpl_cmpl = [None] + [- D_cap_cmpl_cmpl[i] * d_dx(y_minus = sv_1D[f's_cmpl_{i}'], y_plus = sv_1D[f's_cmpl_{i + 1}'],
                                                         dx = H_mpl_node / 2)
                            for i in range(1, nb_mpl)]
    Jl_cmpl_cgdl = - D_cap_cmpl_cgdl * d_dx(y_minus = sv_1D[f's_cmpl_{nb_mpl}'], y_plus = sv_1D['s_cgdl_1'],
                                            dx_minus = H_mpl_node / 2, dx_plus = H_gdl_node / 2)
    Jl_cgdl_cgdl = [None] + [- D_cap_cgdl_cgdl[i] * d_dx(y_minus = sv_1D[f's_cgdl_{i}'], y_plus = sv_1D[f's_cgdl_{i + 1}'],
                                                         dx = H_gdl_node / 2)
                             for i in range(1, nb_gdl)]
    Jl_cgdl_cgc = theta_l_rem * epsilon_gdl * sv_1D[f's_cgdl_{nb_gdl}'] * max((Pcap_cgdl + rho_cgc * v_c ** 2 / 2), 0)
    ...
    Jv_agc_agdl = h_a(Pagc, T_des, Wagc, Hagc) * (C_v_agc - sv_1D['C_v_agdl_1'])                                           # This equation is also calcultaed in velocity.py.
    Jv_cgdl_cgc = h_c(Pcgc, T_des, Wcgc, Hcgc) * (sv_1D[f'C_v_cgdl_{nb_gdl}'] - C_v_cgc)                                   # This equation is also calcultaed in velocity.py.

    # Conductive vapor flows
    #   Anode side
    Jv_agdl_agdl = [None] + [- Da_eff_agdl_agdl[i] * d_dx(y_minus = sv_1D[f'C_v_agdl_{i}'], y_plus = sv_1D[f'C_v_agdl_{i + 1}'],
                                                          dx = H_gdl_node / 2)
                             for i in range(1, nb_gdl)]
    Jv_agdl_ampl = - Da_eff_agdl_ampl * d_dx(y_minus = sv_1D[f'C_v_agdl_{nb_gdl}'], y_plus = sv_1D['C_v_ampl_1'],
                                             dx_minus = H_gdl_node / 2, dx_plus = H_mpl_node / 2)
    Jv_ampl_ampl = [None] + [- Da_eff_ampl_ampl[i] * d_dx(y_minus = sv_1D[f'C_v_ampl_{i}'], y_plus = sv_1D[f'C_v_ampl_{i + 1}'],
                                                          dx = H_mpl_node / 2)
                             for i in range(1, nb_mpl)]
    Jv_ampl_acl = - Da_eff_ampl_acl * d_dx(y_minus = sv_1D[f'C_v_ampl_{nb_mpl}'], y_plus = C_v_acl,
                                           dx_minus = H_mpl_node / 2, dx_plus = Hacl / 2)

    #   Cathode side
    Jv_ccl_cmpl = - Dc_eff_ccl_cmpl * d_dx(y_minus = C_v_ccl, y_plus = sv_1D['C_v_cmpl_1'],
                                           dx_minus = Hccl / 2, dx_plus = H_mpl_node / 2)
    Jv_cmpl_cmpl = [None] + [- Dc_eff_cmpl_cmpl[i] * d_dx(y_minus = sv_1D[f'C_v_cmpl_{i}'], y_plus = sv_1D[f'C_v_cmpl_{i + 1}'],
                                                          dx = H_mpl_node / 2)
                             for i in range(1, nb_mpl)]
    Jv_cmpl_cgdl = - Dc_eff_cmpl_cgdl * d_dx(y_minus = sv_1D[f'C_v_cmpl_{nb_mpl}'], y_plus = sv_1D['C_v_cgdl_1'],
                                             dx_minus = H_mpl_node / 2, dx_plus = H_gdl_node / 2)
    Jv_cgdl_cgdl = [None] + [- Dc_eff_cgdl_cgdl[i] * d_dx(y_minus = sv_1D[f'C_v_cgdl_{i}'], y_plus = sv_1D[f'C_v_cgdl_{i + 1}'],
                                                          dx = H_gdl_node / 2)
                             for i in range(1, nb_gdl)]
    ...
    J_H2_agc_agdl = h_a(Pagc, T_des, Wagc, Hagc) * (C_H2_agc - sv_1D['C_H2_agdl_1'])                                       # This equation is also calcultaed in velocity.py.
    J_O2_cgdl_cgc = h_c(Pcgc, T_des, Wcgc, Hcgc) * (sv_1D[f'C_O2_cgdl_{nb_gdl}'] - C_O2_cgc)                               # This equation is also calcultaed in velocity.py.

    # Conductive H2 and O2 flows
    #   Anode side
    J_H2_agdl_agdl = [None] + [- Da_eff_agdl_agdl[i] * d_dx(y_minus = sv_1D[f'C_H2_agdl_{i}'], y_plus = sv_1D[f'C_H2_agdl_{i + 1}'],
                                                            dx = H_gdl_node / 2)
                               for i in range(1, nb_gdl)]
    J_H2_agdl_ampl = - Da_eff_agdl_ampl * d_dx(y_minus = sv_1D[f'C_H2_agdl_{nb_gdl}'], y_plus = sv_1D['C_H2_ampl_1'],
                                               dx_minus = H_gdl_node / 2, dx_plus = H_mpl_node / 2)
    J_H2_ampl_ampl = [None] + [- Da_eff_ampl_ampl[i] * d_dx(y_minus = sv_1D[f'C_H2_ampl_{i}'], y_plus = sv_1D[f'C_H2_ampl_{i + 1}'],
                                                            dx = H_mpl_node / 2)
                               for i in range(1, nb_mpl)]
    J_H2_ampl_acl = - Da_eff_ampl_acl * d_dx(y_minus = sv_1D[f'C_H2_ampl_{nb_mpl}'], y_plus = C_H2_acl,
                                             dx_minus = H_mpl_node / 2, dx_plus = Hacl / 2)

    #   Cathode side
    J_O2_ccl_cmpl = - Dc_eff_ccl_cmpl * d_dx(y_minus = C_O2_ccl, y_plus = sv_1D['C_O2_cmpl_1'],
                                             dx_minus = Hccl / 2, dx_plus = H_mpl_node / 2)
    J_O2_cmpl_cmpl = [None] + [- Dc_eff_cmpl_cmpl[i] * d_dx(y_minus = sv_1D[f'C_O2_cmpl_{i}'], y_plus = sv_1D[f'C_O2_cmpl_{i + 1}'],
                                                            dx = H_mpl_node / 2)
                               for i in range(1, nb_mpl)]
    J_O2_cmpl_cgdl = - Dc_eff_cmpl_cgdl * d_dx(y_minus = sv_1D[f'C_O2_cmpl_{nb_mpl}'], y_plus = sv_1D['C_O2_cgdl_1'],
                                               dx_minus = H_mpl_node / 2, dx_plus = H_gdl_node / 2)
    J_O2_cgdl_cgdl = [None] + [- Dc_eff_cgdl_cgdl[i] * d_dx(y_minus = sv_1D[f'C_O2_cgdl_{i}'], y_plus = sv_1D[f'C_O2_cgdl_{i + 1}'],
                                                            dx = H_gdl_node / 2)
                               for i in range(1, nb_gdl)]
    ...
    #   Anode side
    Sl_agdl = [None] + [Svl(element='anode', s=sv_1D[f's_agdl_{i}'], C_v=sv_1D[f'C_v_agdl_{i}'],
                            Ctot=sv_1D[f'C_v_agdl_{i}'] + sv_1D[f'C_H2_agdl_{i}'] + C_N2_agc,
                            T=sv_1D[f'T_agdl_{i}'], epsilon=epsilon_gdl) for i in range(1, nb_gdl + 1)]
    Sl_ampl = [None] + [Svl(element='anode', s=sv_1D[f's_ampl_{i}'], C_v=sv_1D[f'C_v_ampl_{i}'],
                            Ctot=sv_1D[f'C_v_ampl_{i}'] + sv_1D[f'C_H2_ampl_{i}'] + C_N2_agc,
                            T=sv_1D[f'T_ampl_{i}'], epsilon=epsilon_mpl) for i in range(1, nb_mpl + 1)]
    Sl_acl = Svl(element='anode', s=s_acl, C_v=C_v_acl, Ctot=C_v_acl + C_H2_acl + C_N2_agc, T=T_acl,
                 epsilon=epsilon_cl(sv_1D['lambda_acl'], sv_1D['T_acl'], Hacl))
    #   Cathode side
    Sl_ccl = Svl(element='cathode', s=s_ccl, C_v=C_v_ccl, Ctot=C_v_ccl + C_O2_ccl + C_N2_cgc, T=T_ccl,
                 epsilon=epsilon_cl(sv_1D['lambda_ccl'], sv_1D['T_ccl'], Hccl))
    Sl_cmpl = [None] + [Svl(element='cathode', s=sv_1D[f's_cmpl_{i}'], C_v=sv_1D[f'C_v_cmpl_{i}'],
                            Ctot=sv_1D[f'C_v_cmpl_{i}'] + sv_1D[f'C_O2_cmpl_{i}'] + C_N2_cgc,
                            T=sv_1D[f'T_cmpl_{i}'], epsilon=epsilon_mpl) for i in range(1, nb_mpl + 1)]
    Sl_cgdl = [None] + [Svl(element='cathode', s=sv_1D[f's_cgdl_{i}'], C_v=sv_1D[f'C_v_cgdl_{i}'],
                            Ctot=sv_1D[f'C_v_cgdl_{i}'] + sv_1D[f'C_O2_cgdl_{i}'] + C_N2_cgc,
                            T=sv_1D[f'T_cgdl_{i}'], epsilon=epsilon_gdl) for i in range(1, nb_gdl + 1)]
