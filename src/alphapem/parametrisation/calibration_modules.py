...
# Importing functions
from alphapem.config.current_densities import polarization_current_for_calibration
...
def save_calibration_results(convergence, ga_instance, solution, varbound, sim_error, type_fuel_cell):
    ...
    root_folder = "results"
    subfolder_name = type_fuel_cell.split('_', 1)[0] if '_' in type_fuel_cell else type_fuel_cell
    ...
    counter = 1
    while True:
        txt_candidate = folder_path / f"{base_stem}_{counter}.txt"
    ...
    with open(str(file_path), "w") as file:
        ...
        if getattr(ga_instance, "best_solution_generation", -1) != -1:
    ...
    ga_instance.save(filename=str(folder_path / f"{base_stem}_{counter}"))

    # Remove ongoing pickle at project root if exists
    ongoing = project_root / "parameter_calibration_ongoing.pkl"
    if ongoing.exists():
        try:
            ongoing.unlink()
        except Exception:
