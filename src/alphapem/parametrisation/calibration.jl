...
function calibrate(cfg::CalibrationConfig)::CalibrationResult
    ...
    nb_gc = ref_cfg.numerical_parameters.nb_gc
    parameter_bounds = bounds_for_fuel_cell(ref_cfg.type_fuel_cell, ref_cfg.voltage_zone;
                                            year=ref_cfg.year, nb_gc=nb_gc) # Retrieve parameter constraints
    base_params = get_reference_config(ref_cfg.type_fuel_cell; year=ref_cfg.year, nb_gc=nb_gc) # Load baseline physical parameters
    ...
    fuel_cells       = [create_fuelcell(sc.type_fuel_cell, sc.voltage_zone; year=sc.year, nb_gc=sc.numerical_parameters.nb_gc) for sc in cfg.simulation_configs]
    current_profiles = [create_current(PolarizationCalibrationParams(), fc) for fc in fuel_cells]

    # 3. Define fitness function
    # When parallel=true, we use a batch fitness function evaluated with Threads.@spawn in Julia.
    ...
    #
    # Julia functions are wrapped in Python lambdas so that PyGAD's inspect.signature() can count their
    # parameters and identify batch vs. single mode.
    # Python 3.13 requires explicit globals when eval() is called outside a Python frame (i.e. from Julia).
    ...
    # Convert initial population to a Python list of lists (PythonCall does not auto-convert Julia vectors)
    initial_population_py = pylist([pylist(collect(ind)) for ind in initial_population])
    ...
    # Gene space: Python list of dicts with 'low' and 'high' for each gene
    gene_space = pylist([pydict(["low" => lower_bounds[i], "high" => upper_bounds[i]]) for i in 1:num_params])
    ...
    ga_instance = pygad.GA(
        ...
        keep_elitism           = ga_config.elitism,            # Keep the best individuals from the previous generation.
    ...
    best_fitness    = 1.0 / pyconvert(Float64, best_fitness_py) # Convert 1/RMSE back to RMSE
    optimized_genes = pyconvert(Vector{Float64}, best_sol) # Extract best individual found (global best)
    ...
    final_population_matrix = pyconvert(Matrix{Float64}, ga_instance.population) # Shape: (pop_size, num_genes)
    final_fitness_values    = pyconvert(Vector{Float64}, ga_instance.last_generation_fitness) # Shape: (pop_size,)
