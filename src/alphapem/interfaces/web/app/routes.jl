...
route("/api/simulate/step"; method="POST") do
    try
        ...
        # Run simulation
        result = SimulatorBackend.run_step_simulation(config; params=params)
...
route("/api/simulate/polarization"; method="POST") do
    try
        ...
        result = SimulatorBackend.run_polarization_simulation(config; params=params)
...
route("/api/simulate/eis"; method="POST") do
    try
        ...
        result = SimulatorBackend.run_eis_simulation(config; params=params)
...
# - plots: List of plot image URLs (generated by CairoMakie)
...
# API endpoint: Serve an interactive plot HTML generated by WGLMakie.
#
# Parameters:
# - result_id: The simulation result ID
# - filename: The HTML filename (e.g. plot_main.html, plot_gc_profiles.html)
route("/api/plots/:result_id/:filename"; method="GET") do
    result_id::String = Router.params()[:result_id]
    filename::String  = Router.params()[:filename]

    # Only allow safe filenames (no path traversal).
    if occursin(r"[/\\]", filename) || startswith(filename, ".")
        return HTTP.Response(400, "Invalid filename")
    end

    plot_path = joinpath(SimulatorBackend.PLOTS_DIR, result_id, filename)
    if isfile(plot_path)
        # Serve HTML files with proper MIME type
        if endswith(filename, ".html")
            html_content = read(plot_path, String)
            HTTP.Response(200, ["Content-Type" => "text/html; charset=utf-8"], html_content)
        else
            # Serve other file types (unlikely but safe)
            content = read(plot_path)
            content_type = endswith(filename, ".json") ? "application/json" : "application/octet-stream"
            HTTP.Response(200, ["Content-Type" => content_type], content)
        end
    else
        HTTP.Response(404, "Plot not found: $filename")
    end
end
...
route("/api/download/:result_id"; method="GET") do
    result_id::String = Router.params()[:result_id]
    format = get(Router.params(), :format, "csv")  # Default to CSV
    index = get(Router.params(), :index, nothing)

    try
        # Get export data and suggested filename
        export_res = SimulatorBackend.export_results(result_id, format, index=index)
        export_data = export_res.data
        filename = export_res.filename

        # Determine content type
        content_type = if format == "pdf"
            "application/pdf"
