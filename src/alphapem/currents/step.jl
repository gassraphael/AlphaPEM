...
"""
    solver_tstops(c::StepCurrent, tspan)

Return the stop times associated with the step profile.
...
Only the beginning of each ramp is retained as a forced stop point.
"""
function solver_tstops(c::StepCurrent, tspan::Tuple{<:Real, <:Real})::Vector{Float64}
    times = Float64[0.0, c.delta_t_ini]
