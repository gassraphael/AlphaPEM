...
def calculate_dyn_dissoved_water_evolution_inside_MEA(dif_eq_1D, sv_1D, Hmem, Hacl, Hccl, S_abs, J_lambda, Sp,
                                                      **kwargs):
    """
...
def calculate_dyn_liquid_water_evolution_inside_MEA(dif_eq_1D, sv_1D, Aact, Wagc, Wcgc, Lgc, nb_channel_in_gc, Hgdl, Hmpl, Hacl,
    ...
    if nb_gdl == 1:
        ...
        for i in range(2, nb_gdl):
            dif_eq_1D[f'ds_agdl_{i} / dt'] = 1 / (rho_H2O_l(sv_1D[f'T_agdl_{i}']) * epsilon_gdl) * \
                                             ((Jl['agdl_agdl'][i - 1] - Jl['agdl_agdl'][i]) / (Hgdl / nb_gdl) +
                                           M_H2O * Sl['agdl'][i])
        dif_eq_1D[f'ds_agdl_{nb_gdl} / dt'] = 1 / (rho_H2O_l(sv_1D[f'T_agdl_{nb_gdl}']) * epsilon_gdl) * \
    ...
    if nb_mpl == 1:
        ...
        for i in range(2, nb_mpl):
            dif_eq_1D[f'ds_ampl_{i} / dt'] = 1 / (rho_H2O_l(sv_1D[f'T_ampl_{i}']) * epsilon_mpl) * \
                                             ((Jl['ampl_ampl'][i - 1] - Jl['ampl_ampl'][i]) / (Hmpl / nb_mpl) +
                                           M_H2O * Sl['ampl'][i])
        dif_eq_1D[f'ds_ampl_{nb_mpl} / dt'] = 1 / (rho_H2O_l(sv_1D[f'T_ampl_{nb_mpl}']) * epsilon_mpl) * \
    ...
    if nb_mpl == 1:
        ...
        dif_eq_1D['ds_cmpl_1 / dt'] = 1 / (rho_H2O_l(sv_1D['T_cmpl_1']) * epsilon_mpl) * \
                                      ((Jl['ccl_cmpl'] - Jl['cmpl_cmpl'][1]) / (Hmpl / nb_mpl) + M_H2O * Sl['cmpl'][1])
        for i in range(2, nb_mpl):
            dif_eq_1D[f'ds_cmpl_{i} / dt'] = 1 / (rho_H2O_l(sv_1D[f'T_cmpl_{i}']) * epsilon_mpl) * \
                                             ((Jl['cmpl_cmpl'][i - 1] - Jl['cmpl_cmpl'][i]) / (Hmpl / nb_mpl) +
                                           M_H2O * Sl['cmpl'][i])
        dif_eq_1D[f'ds_cmpl_{nb_mpl} / dt'] = 1 / (rho_H2O_l(sv_1D[f'T_cmpl_{nb_mpl}']) * epsilon_mpl) * \
    ...
    if nb_gdl == 1:
        ...
        dif_eq_1D['ds_cgdl_1 / dt'] = 1 / (rho_H2O_l(sv_1D['T_cgdl_1']) * epsilon_gdl) * \
                                      ((Jl['cmpl_cgdl'] - Jl['cgdl_cgdl'][1]) / (Hgdl / nb_gdl) + M_H2O * Sl['cgdl'][1])
        for i in range(2, nb_gdl):
            dif_eq_1D[f'ds_cgdl_{i} / dt'] = 1 / (rho_H2O_l(sv_1D[f'T_cgdl_{i}']) * epsilon_gdl) * \
                                             ((Jl['cgdl_cgdl'][i - 1] - Jl['cgdl_cgdl'][i]) / (Hgdl / nb_gdl) + M_H2O * Sl['cgdl'][i])
        dif_eq_1D[f'ds_cgdl_{nb_gdl} / dt'] = 1 / (rho_H2O_l(sv_1D[f'T_cgdl_{nb_gdl}']) * epsilon_gdl) * \
                                              ((Jl['cgdl_cgdl'][nb_gdl - 1] - Jl_cgdl_cgc_red) / (Hgdl / nb_gdl) + M_H2O * Sl['cgdl'][nb_gdl])


def calculate_dyn_vapor_evolution_inside_MEA(dif_eq_1D, sv_1D, Aact, Wagc, Wcgc, Lgc, nb_channel_in_gc, Hgdl, Hmpl, Hacl,
    ...
    if nb_gdl == 1:
        ...
        dif_eq_1D['dC_v_agdl_1 / dt'] = 1 / (epsilon_gdl * (1 - sv_1D['s_agdl_1'])) * \
                                        ((Jv_agc_agdl_red - Jv['agdl_agdl'][1]) / (Hgdl / nb_gdl) + Sv['agdl'][1])
        for i in range(2, nb_gdl):
            dif_eq_1D[f'dC_v_agdl_{i} / dt'] = 1 / (epsilon_gdl * (1 - sv_1D[f's_agdl_{i}'])) * \
                                               ((Jv['agdl_agdl'][i - 1] - Jv['agdl_agdl'][i]) / (Hgdl / nb_gdl) + Sv['agdl'][i])
        dif_eq_1D[f'dC_v_agdl_{nb_gdl} / dt'] = 1 / (epsilon_gdl * (1 - sv_1D[f's_agdl_{nb_gdl}'])) * \
                                                ((Jv['agdl_agdl'][nb_gdl - 1] - Jv['agdl_ampl']) / (Hgdl / nb_gdl) + Sv['agdl'][nb_gdl])
    #       Inside the AMPL
    if nb_mpl == 1:
        dif_eq_1D['dC_v_ampl_1 / dt'] = 1 / (epsilon_mpl * (1 - sv_1D['s_ampl_1'])) * ((Jv['agdl_ampl'] - Jv['ampl_acl']) / Hmpl +
                                                                                       Sv['ampl'][1])
    elif nb_mpl == 2:
        ...
        dif_eq_1D['dC_v_ampl_1 / dt'] = 1 / (epsilon_mpl * (1 - sv_1D['s_ampl_1'])) * \
                                        ((Jv['agdl_ampl'] - Jv['ampl_ampl'][1]) / (Hmpl / nb_mpl) + Sv['ampl'][1])
        for i in range(2, nb_mpl):
            dif_eq_1D[f'dC_v_ampl_{i} / dt'] = 1 / (epsilon_mpl * (1 - sv_1D[f's_ampl_{i}'])) * \
                                               ((Jv['ampl_ampl'][i - 1] - Jv['ampl_ampl'][i]) / (Hmpl / nb_mpl) + Sv['ampl'][i])
        dif_eq_1D[f'dC_v_ampl_{nb_mpl} / dt'] = 1 / (epsilon_mpl * (1 - sv_1D[f's_ampl_{nb_mpl}'])) * \
    ...
    #       Inside the CMPL
    if nb_mpl == 1:
        dif_eq_1D['dC_v_cmpl_1 / dt'] = 1 / (epsilon_mpl * (1 - sv_1D['s_cmpl_1'])) * ((Jv['ccl_cmpl'] - Jv['cmpl_cgdl']) / Hmpl +
                                                                                       Sv['cmpl'][1])
    elif nb_mpl == 2:
        ...
        dif_eq_1D['dC_v_cmpl_1 / dt'] = 1 / (epsilon_mpl * (1 - sv_1D['s_cmpl_1'])) * \
                                        ((Jv['ccl_cmpl'] - Jv['cmpl_cmpl'][1]) / (Hmpl / nb_mpl) + Sv['cmpl'][1])
        for i in range(2, nb_mpl):
            dif_eq_1D[f'dC_v_cmpl_{i} / dt'] = 1 / (epsilon_mpl * (1 - sv_1D[f's_cmpl_{i}'])) * \
                                               ((Jv['cmpl_cmpl'][i - 1] - Jv['cmpl_cmpl'][i]) / (Hmpl / nb_mpl) + Sv['cmpl'][i])
        dif_eq_1D[f'dC_v_cmpl_{nb_mpl} / dt'] = 1 / (epsilon_mpl * (1 - sv_1D[f's_cmpl_{nb_mpl}'])) * \
    ...
    if nb_gdl == 1:
        dif_eq_1D['dC_v_cgdl_1 / dt'] = 1 / (epsilon_gdl * (1 - sv_1D['s_cgdl_1'])) * ((Jv['cmpl_cgdl'] - Jv_cgdl_cgc_red) / Hgdl +
                                                                                       Sv['cgdl'][1])
    elif nb_gdl == 2:
        ...
        dif_eq_1D['dC_v_cgdl_1 / dt'] = 1 / (epsilon_gdl * (1 - sv_1D['s_cgdl_1'])) * \
                                        ((Jv['cmpl_cgdl'] - Jv['cgdl_cgdl'][1]) / (Hgdl / nb_gdl) + Sv['cgdl'][1])
        for i in range(2, nb_gdl):
            dif_eq_1D[f'dC_v_cgdl_{i} / dt'] = 1 / (epsilon_gdl * (1 - sv_1D[f's_cgdl_{i}'])) * \
                                               ((Jv['cgdl_cgdl'][i - 1] - Jv['cgdl_cgdl'][i]) / (Hgdl / nb_gdl) + Sv['cgdl'][i])
        dif_eq_1D[f'dC_v_cgdl_{nb_gdl} / dt'] = 1 / (epsilon_gdl * (1 - sv_1D[f's_cgdl_{nb_gdl}'])) * \
                                                ((Jv['cgdl_cgdl'][nb_gdl - 1] - Jv_cgdl_cgc_red) / (Hgdl / nb_gdl) + Sv['cgdl'][nb_gdl])


def calculate_dyn_H2_O2_N2_evolution_inside_MEA(dif_eq_1D, sv_1D, Aact, Wagc, Wcgc, Lgc, nb_channel_in_gc, Hgdl, Hmpl, Hacl,
    ...
    if nb_gdl == 1:
        ...
        dif_eq_1D['dC_H2_agdl_1 / dt'] = 1 / (epsilon_gdl * (1 - sv_1D['s_agdl_1'])) * \
                                         (J_H2_agc_agdl_red - J_H2['agdl_agdl'][1]) / (Hgdl / nb_gdl)
        for i in range(2, nb_gdl):
            dif_eq_1D[f'dC_H2_agdl_{i} / dt'] = 1 / (epsilon_gdl * (1 - sv_1D[f's_agdl_{i}'])) * \
                                                (J_H2['agdl_agdl'][i - 1] - J_H2['agdl_agdl'][i]) / (Hgdl / nb_gdl)
        dif_eq_1D[f'dC_H2_agdl_{nb_gdl} / dt'] = 1 / (epsilon_gdl * (1 - sv_1D[f's_agdl_{nb_gdl}'])) * \
    ...
    if nb_mpl == 1:
        ...
        dif_eq_1D['dC_H2_ampl_1 / dt'] = 1 / (epsilon_mpl * (1 - sv_1D['s_ampl_1'])) * \
                                         (J_H2['agdl_ampl'] - J_H2['ampl_ampl'][1]) / (Hmpl / nb_mpl)
        for i in range(2, nb_mpl):
            dif_eq_1D[f'dC_H2_ampl_{i} / dt'] = 1 / (epsilon_mpl * (1 - sv_1D[f's_ampl_{i}'])) * \
                                                (J_H2['ampl_ampl'][i - 1] - J_H2['ampl_ampl'][i]) / (Hmpl / nb_mpl)
        dif_eq_1D[f'dC_H2_ampl_{nb_mpl} / dt'] = 1 / (epsilon_mpl * (1 - sv_1D[f's_ampl_{nb_mpl}'])) * \
    ...
    if nb_mpl == 1:
        ...
        dif_eq_1D['dC_O2_cmpl_1 / dt'] = 1 / (epsilon_mpl * (1 - sv_1D['s_cmpl_1'])) * \
                                         (J_O2['ccl_cmpl'] - J_O2['cmpl_cmpl'][1]) / (Hmpl / nb_mpl)
        for i in range(2, nb_mpl):
            dif_eq_1D[f'dC_O2_cmpl_{i} / dt'] = 1 / (epsilon_mpl * (1 - sv_1D[f's_cmpl_{i}'])) * \
                                                (J_O2['cmpl_cmpl'][i - 1] - J_O2['cmpl_cmpl'][i]) / (Hmpl / nb_mpl)
        dif_eq_1D[f'dC_O2_cmpl_{nb_mpl} / dt'] = 1 / (epsilon_mpl * (1 - sv_1D[f's_cmpl_{nb_mpl}'])) * \
    ...
    if nb_gdl == 1:
        ...
        dif_eq_1D['dC_O2_cgdl_1 / dt'] = 1 / (epsilon_gdl * (1 - sv_1D['s_cgdl_1'])) * \
                                         (J_O2['cmpl_cgdl'] - J_O2['cgdl_cgdl'][1]) / (Hgdl / nb_gdl)
        for i in range(2, nb_gdl):
            dif_eq_1D[f'dC_O2_cgdl_{i} / dt'] = 1 / (epsilon_gdl * (1 - sv_1D[f's_cgdl_{i}'])) * \
                                                (J_O2['cgdl_cgdl'][i - 1] - J_O2['cgdl_cgdl'][i]) / (Hgdl / nb_gdl)
        dif_eq_1D[f'dC_O2_cgdl_{nb_gdl} / dt'] = 1 / (epsilon_gdl * (1 - sv_1D[f's_cgdl_{nb_gdl}'])) * \
                                                 (J_O2['cgdl_cgdl'][nb_gdl - 1] - J_O2_cgdl_cgc_red) / (Hgdl / nb_gdl)


def calculate_dyn_temperature_evolution_inside_MEA(dif_eq_1D, Hgdl, Hmpl, Hacl, Hccl, Hmem, nb_gdl, nb_mpl, rho_Cp0, Jt,
    ...
    if nb_gdl == 1:
        ...
        dif_eq_1D['dT_agdl_1 / dt'] = (1 / rho_Cp0['agdl_1']) * ((Jt['agc_agdl'] - Jt['agdl_agdl_1']) / (Hgdl / nb_gdl) +
                                                                 Q_liq['agdl_1'] + Q_e['agdl_1'])
        for i in range(2, nb_gdl):
            dif_eq_1D[f'dT_agdl_{i} / dt'] = (1 / rho_Cp0[f'agdl_{i}']) * \
                                             ((Jt[f'agdl_agdl_{i - 1}'] - Jt[f'agdl_agdl_{i}']) / (Hgdl / nb_gdl) +
                                           Q_liq[f'agdl_{i}'] + Q_e[f'agdl_{i}'])
        dif_eq_1D[f'dT_agdl_{nb_gdl} / dt'] = (1 / rho_Cp0[f'agdl_{nb_gdl}']) * \
    ...
    if nb_mpl == 1:
        ...
        dif_eq_1D['dT_ampl_1 / dt'] = (1 / rho_Cp0['ampl_1']) * ((Jt['agdl_ampl'] - Jt['ampl_ampl_1']) / (Hmpl / nb_mpl) +
                                                                 Q_liq['ampl_1'] + Q_e['ampl_1'])
        for i in range(2, nb_mpl):
            dif_eq_1D[f'dT_ampl_{i} / dt'] = (1 / rho_Cp0[f'ampl_{i}']) * \
                                             ((Jt[f'ampl_ampl_{i - 1}'] - Jt[f'ampl_ampl_{i}']) / (Hmpl / nb_mpl) +
                                           Q_liq[f'ampl_{i}'] + Q_e[f'ampl_{i}'])
        dif_eq_1D[f'dT_ampl_{nb_mpl} / dt'] = (1 / rho_Cp0[f'ampl_{nb_mpl}']) * \
    ...
    # Inside the membrane
    dif_eq_1D['dT_mem / dt'] = (1 / rho_Cp0['mem']) * \
    ...
    if nb_mpl == 1:
        ...
        dif_eq_1D['dT_cmpl_1 / dt'] = (1 / rho_Cp0['cmpl_1']) * \
                                    ...
                                    Q_liq['cmpl_1'] + Q_e['cmpl_1'])
        for i in range(2, nb_mpl):
            dif_eq_1D[f'dT_cmpl_{i} / dt'] = (1 / rho_Cp0[f'cmpl_{i}']) * \
                                             ((Jt[f'cmpl_cmpl_{i - 1}'] - Jt[f'cmpl_cmpl_{i}']) / (Hmpl / nb_mpl) +
                                           Q_liq[f'cmpl_{i}'] + Q_e[f'cmpl_{i}'])
        dif_eq_1D[f'dT_cmpl_{nb_mpl} / dt'] = (1 / rho_Cp0[f'cmpl_{nb_mpl}']) * \
    ...
    if nb_gdl == 1:
        ...
        dif_eq_1D['dT_cgdl_1 / dt'] = (1 / rho_Cp0['cgdl_1']) * \
                                    ...
                                    Q_liq['cgdl_1'] + Q_e['cgdl_1'])
        for i in range(2, nb_gdl):
            dif_eq_1D[f'dT_cgdl_{i} / dt'] = (1 / rho_Cp0[f'cgdl_{i}']) * \
                                             ((Jt[f'cgdl_cgdl_{i - 1}'] - Jt[f'cgdl_cgdl_{i}']) / (Hgdl / nb_gdl) +
                                           Q_liq[f'cgdl_{i}'] + Q_e[f'cgdl_{i}'])
        dif_eq_1D[f'dT_cgdl_{nb_gdl} / dt'] = (1 / rho_Cp0[f'cgdl_{nb_gdl}']) * \
                                              ((Jt[f'cgdl_cgdl_{nb_gdl - 1}'] - Jt['cgdl_cgc']) / (Hgdl / nb_gdl) +
                                            Q_liq[f'cgdl_{nb_gdl}'] + Q_e[f'cgdl_{nb_gdl}'])


def calculate_dyn_voltage_evolution(dif_eq_1D, i_fc, C_O2_Pt, T_ccl, eta_c, Hccl, i0_c_ref, kappa_c, C_scl, i_n, **kwargs):
