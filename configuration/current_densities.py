...
def EIS_current(t, parameters):
    ...
    t0_EIS, t_new_start, tf_EIS, delta_t_break_EIS, delta_t_measurement_EIS = parameters['t_EIS']  # It is the initial
    ...
    f_power_min, f_power_max, nb_f, nb_points = parameters['f_EIS']  # It is the power of the initial frequency:
    #         f_min = 10**f_power_min, the power of the final frequency, the number of frequencies tested and the number
    #         of points calculated per specific period.
    ...
    if t < t0_EIS:
        ...
        n_inf = np.where(t_new_start <= t)[0][-1]  # It is the number of frequency changes which has been made so far.
...
if __name__ == "__main__":
    ...
    #                           power of the initial frequency 'f_power_min': f_min = 10**f_power_min, the power of the
    #                           final frequency 'f_power_max', the number of frequencies tested 'nb_f' and the number of
    ...
    #        EIS current density
    for i in range(len(t_new_start)):  # The EIS curve is displayed for each frequency change in order to reduce the
        #                               number of points to calculate.
        t0_EIS_temp = t_new_start[i]
        tf_EIS_temp = t_new_start[i] + delta_t_break_EIS[i] + delta_t_measurement_EIS[i]
        n_inf = np.where(t_new_start <= t0_EIS_temp)[0][-1]  # It is the number of frequency changes
        #                                                      which has been made so far.
        n = int(f[n_inf] * (tf_EIS_temp - t0_EIS_temp) * nb_points)  # It is the number of points to calculate.
