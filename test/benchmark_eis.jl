...
"""
Performance benchmark for AlphaPEM using an EIS signal.
...
results/benchmark/benchmark_eis.csv.
"""
...
function main()
    ...
    mkpath(out_dir)
    out_csv = generate_output_path(out_dir, "benchmark_eis.csv")

    rows = NamedTuple[]
    println("First launch (includes compilation)...")
    m0 = @timed run_simulation(make_eis_config())
    push!(rows, (
    ...
    for i in 1:runs
        ...
        m = @timed run_simulation(make_eis_config())
        push!(rows, (
            timestamp = Dates.format(now(), dateformat"yyyy-mm-ddTHH:MM:SS"),
            phase = "measured",
            run_index = i,
